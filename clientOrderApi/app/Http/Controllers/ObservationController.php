...
class ObservationController extends Controller
{
    ...
    public function store(Request $request)
    {
        $token = Session::get('token');
        $url = env("URL_BASE_API", "http://localhost:8000");
        $response = Http::acceptJson()->withToken($token)->post($url . "/observation", [
    ...
    public function edit(string $id)
    {
        $token = Session::get('token');
        $url = env('URL_BASE_API', "http://localhost:8000");
        $response = Http::acceptJson()->withToken($token)->get($url . '/observation/'. $id);
        if ($response->successful()) {
    ...
    public function update(Request $request, string $id)
    {
        $token = Session::get('token');
        $url = env('URL_BASE_API', "http://localhost:8000");
        $response = Http::acceptJson()->withToken($token)->put($url . '/observation/'. $id, [
            'id' => $request->id,
            'description' => $request->description
        ]);
        if ($response->successful()) {
    ...
    public function destroy(string $id)
    {
        $token = Session::get('token');
        $url = env('URL_BASE_API', "http://localhost:8000");
        $response = Http::acceptJson()->withToken($token)->delete($url . '/observation/'. $id);
        if ($response->successful()) {
